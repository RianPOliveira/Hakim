  ...
  return (
    <div className="results-card">
      ...
      <div className="details">
        <div><h3>✅ Pontos Fortes:</h3><ul>{result.pontos_fortes?.map((point, index) => <li key={index}>{point}</li>)}</ul></div>
        <div><h3>⚠️ Pontos de Melhoria:</h3><ul>{result.pontos_melhoria?.map((point, index) => <li key={index}>{point}</li>)}</ul></div>
      </div>
    ...
    <div className="tab-content">
      ...
      <form onSubmit={handleSubmit}>
        <label htmlFor="single-file-upload" className="upload-area">
          <i className="fas fa-cloud-upload-alt"></i><p>{fileName || 'Clique para selecionar um arquivo'}</p><span>Suporta: Imagens, Áudio, Vídeo e PDF</span>
        </label>
        <input id="single-file-upload" type="file" onChange={handleFileChange} accept="image/*,audio/*,video/*,.pdf" />
        <div className="criteria-input"><label>Critérios de Avaliação</label><input type="text" value={criteria} onChange={(e) => setCriteria(e.target.value)} placeholder="Critérios de avaliação" /></div>
    ...
    <div className="tab-content">
      ...
      <form onSubmit={handleSubmit}>
        <label htmlFor="multi-file-upload" className="upload-area">
          <i className="fas fa-cloud-upload-alt"></i><p>{files.length > 0 ? `${files.length} arquivos selecionados` : 'Clique para selecionar arquivos'}</p><span>Suporta: Imagens, Áudio, Vídeo e PDF</span>
        </label>
        ...
        <div className="criteria-input"><label>Critérios da Competição</label><input type="text" value={criteria} onChange={(e) => setCriteria(e.target.value)} placeholder="Critérios da competição" /></div>
